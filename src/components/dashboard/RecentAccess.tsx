    ...
    <div className="divide-y divide-gray-100">
      {logs.map((log) => (
        <div key={log.id} className="flex items-center gap-4 py-3 first:pt-0 last:pb-0">
          <div className="flex items-center justify-center w-9 h-9 rounded-full bg-gray-100 text-gray-600 text-xs font-semibold shrink-0">
            {getInitials(log.member_name)}
          </div>
          <div className="flex-1 min-w-0">
            <p className="text-sm font-medium text-gray-900 truncate">{log.member_name}</p>
            <p className="text-xs text-gray-400 mt-0.5">{formatDateTime(log.entry_time)}</p>
          </div>
          {log.access_granted ? (
